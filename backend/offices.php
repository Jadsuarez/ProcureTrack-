...
{
    $stmt = $pdo->query(
        'SELECT o.id, o.slug, o.label, o.is_system, o.created_by, o.created_at,
...
try {
    ...
    if ($action === 'create') {
        ...
        $insert = $pdo->prepare(
            'INSERT INTO offices (slug, label, is_system, created_by) VALUES (?, ?, 0, ?)'
        );
        $insert->execute([$slug, $label, $actor]);
        refreshOfficeCache();
